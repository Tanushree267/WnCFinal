...
// getShow - fix API call backticks
const getShow = async () => {
  try {
...
const renderSeats = (row, count = 9) => (
  <div key={row} className='flex gap-2 mt-2'>
    <div className='flex flex-wrap items-center justify-center gap-2'>
      {Array.from({ length: count }).map((_, i) => {
        const seatId = `${row}${i + 1}` // <-- backticks here
        ...
        return (
          <button
            ...
            className={`h-8 w-8 rounded border border-primary/60 cursor-pointer
              ...
              ${isOccupied ? "bg-red-500 opacity-50 cursor-not-allowed" : ""}`}>
            {seatId}
          </button>
        )
      })}
  ...
  onClick={() => setSelectedTime({ showId: showItem._id, time: showItem.showDateTime })}
  className={`flex items-center gap-2 px-6 py-2 w-max rounded-r-md cursor-pointer transition ${selectedTime?.time === showItem.showDateTime ? "bg-primary text-white" : "hover:bg-primary/20"}`}>
      ...
      <div className='relative flex flex-1 flex-col items-center max-md:mt-16'>
        ...
        <div className='mt-8 w-full flex justify-center'>
          <div className='bg-primary/10 border border-primary/20 rounded-lg p-6 w-80 md:w-96'>
            ...
            <p className='font-semibold text-lg'>{selectedTime ? isoTimeFormat(selectedTime.time) : '—'}</p>

            <div className='mt-4'>
              <p className='text-sm text-gray-400'>Seats</p>
              <p className='font-medium'>{selectedSeats.length ? selectedSeats.join(', ') : '—'}</p>
            </div>

             <div>
              <p className='text-sm text-gray-400'>Show Price</p>
              <p className='font-semibold text-lg'>₹{currentShow?.showPrice || 0}</p>
            </div>
...
console.log("PAYLOAD:", {
  ...
  }} className='flex items-center gap-1 mt-6 px-6 py-3 text-sm bg-primary hover:bg-primary-dull transition rounded-full font-medium cursor-pointer active:scale-95 w-full justify-center'> Proceed to checkout
